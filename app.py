...
def render_privacy_policy() -> None:
    ...
    st.caption("Last updated: July 7, 2026")
    st.markdown(
        """
        This privacy policy describes how the Advertiser Dashboard Automation
        application handles information when it is used by SME to prepare
        advertiser campaign performance dashboards and reports.

        ### Information the application accesses

        The application may access campaign, advertiser, creative, delivery,
        analytics, and reporting data from connected business systems, including
        Google Ad Manager, Google Analytics, Google Sheets, HubSpot, and
        Webinar.net, depending on which integrations are configured. This data is
        used to generate internal dashboards, client-facing campaign performance
        views, and downloadable reports.

        ### Google API data

        When connected to Google services, the application uses authorized Google
        API access only to retrieve or update reporting data needed for the
        dashboard. Google Ad Manager access is used to read advertiser, campaign,
        creative, impression, click, and related delivery data. Google Sheets
        access, when configured, is used to read and update advertiser source and
        reporting tables used by the dashboard.

        The application does not sell Google user data, use Google user data for
        advertising, or transfer Google user data to unrelated third parties. Data
        is used only for SME business reporting and advertiser dashboard
        workflows.

        ### How information is stored

        The application may store configuration files, OAuth credentials, service
        credentials, report exports, and cached reporting data in the local or
        hosted environment where the application is deployed. Access to those
        files should be restricted to authorized SME personnel and systems.

        ### Sharing

        Reports generated by the application may be shared with SME staff,
        authorized clients, or business partners for campaign reporting purposes.
        Raw credentials and tokens should not be shared.

        ### Security

        SME uses reasonable administrative and technical safeguards to protect
        access to the application, connected accounts, credentials, and generated
        reports. No system can be guaranteed to be completely secure.

        ### Data retention

        Reporting data and generated exports are retained only as long as needed
        for business, reporting, operational, or legal purposes. Local exports and
        cached files may be deleted when they are no longer needed.

        ### Contact

        Questions about this privacy policy or the application's data handling
        can be sent to mschneider@sme.org.
        """
    )
